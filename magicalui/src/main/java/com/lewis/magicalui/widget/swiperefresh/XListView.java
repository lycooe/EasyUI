package com.lewis.magicalui.widget.swiperefresh;
...
import com.lewis.magicalui.adapter.LibraryBaseAdapter;
import com.lewis.magicalui.adapter.LibraryBaseListViewAdapter;
...
//    private IXListViewListener mListener;
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (adapter instanceof LibraryBaseAdapter) {
            ((LibraryBaseAdapter) adapter).setReleaseListener(new LibraryBaseAdapter.ReleaseListener() {
                @Override
        ...
        } else if (adapter instanceof LibraryBaseListViewAdapter) {
            ((LibraryBaseListViewAdapter) adapter).setReleaseListener(new LibraryBaseListViewAdapter.ReleaseListener() {
                @Override
                public void listStopRefresh() {
                    stop();
                }
    ...
    public void setOnRefreshListener(OnRefreshListener listener) {
        ...
        mHeaderLayout.setVisibility(null != mRefreshListener ? View.VISIBLE : View.INVISIBLE);
    }
    ...
    public void setOnLoadMoreListener(boolean autoLoadEnable, OnLoadMoreListener listener) {
        ...
        showOrHideFooter(listener != null);
    }

    public void showOrHideFooter(boolean isShow) {
        end = !isShow;
        if (!isShow || mLoadMoreListener == null) {
    ...
    public void autoRefresh() {
        ...
        if (null != mRefreshListener && !mPullRefreshing) {
...
//        Log.d("XLIST", "height "+height);
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (null != mLoadMoreListener && !mPullLoading) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mLastY = mInitialMotionY = ev.getRawY();
...
//        Log.d("XLIST", "onTouchEvent "+ev.getAction());
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mLastY = mInitialMotionY = ev.getRawY();
                mActivePointerId = ev.getPointerId(0);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                final float deltaY = ev.getY(pointerIndex) - mLastY;
                if (getFirstVisiblePosition() == 0 && (mHeader.getVisibleHeight() > 0 ||
                        deltaY > 0) && null != mRefreshListener) {
...
//                }
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                mInitialMotionY = ev.getY(index) - (mLastY - mInitialMotionY);
...
//                    Log.d("XLIST", "default "+ev.getAction());
                    // invoke refresh
                    if (null != mRefreshListener && mHeader.getVisibleHeight() > mHeaderHeight) {
                        mPullRefreshing = true;
                        mHeader.setState(XHeaderView.STATE_REFRESHING);
                        refresh();
                    }
                    ...
                    // invoke load more.
                    if (null != mLoadMoreListener && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA) {
                        startLoadMore();
                    }
                    resetFooterHeight();
                }
